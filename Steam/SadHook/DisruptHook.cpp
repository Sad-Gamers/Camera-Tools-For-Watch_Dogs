...
void DisruptHook::Initialize() {
	...
	pGameSpeedReadSubProc = (DWORD64)((SadHook::GetImagebase() + 0x45CD80));
	pCallRenderFrameSubProc = (DWORD64)((SadHook::GetImagebase() + 0x95F866));
	pReadActionMapEnum = (DWORD64)((SadHook::GetImagebase() + 0x3C08C3));
	SendShowOrHideAllUIEvent = (SendShowOrHideAllUIEvent_t)((DWORD64)(SadHook::GetImagebase() + 0x11BBEE0));

	//Defining memory locations of instructions/functions we will be overwriting or using (Camera).
	pCameraPositionSubProc = (DWORD64)((SadHook::GetImagebase() + 0xA6E3C0));
	pCameraFOVWrite = (DWORD64)((SadHook::GetImagebase() + 0xA6E3E4));
	pViewMatrixSubProc = (DWORD64)((SadHook::GetImagebase() + 0x2A1150));
	pCallMoveReticleSubProc = (DWORD64)((SadHook::GetImagebase() + 0x1836B3A));
	pLensRenderpassSubProc1 = (DWORD64)((SadHook::GetImagebase() + 0x15EDFE0));
	pLensRenderpassSubProc2 = (DWORD64)((SadHook::GetImagebase() + 0x15BD250));
	pLensRenderpassSubProc3 = (DWORD64)((SadHook::GetImagebase() + 0x15BCA60));
...
void DisruptHook::DisruptHookUpdate() {
	while (true) {
		...
		*/
		pGameLoading = (DWORD64)((*(uint64_t*)((SadHook::GetImagebase() + 0x3B78658)) + 0x1A8));
		pCameraLoaded = (DWORD64)((SadHook::GetImagebase() + 0x3B94260));
		...
		*/
		if (!bGameLoading && bCameraLoaded) {
			...
			pEnvironmentPreset = ((*(uint64_t*)((SadHook::GetImagebase() + 0x3B6FFD8)) + 0x278));
			pTimeOfDay = ((*(uint64_t*)((*(uint64_t*)(SadHook::GetImagebase() + 0x3B70128)) + 0x8) + 0x758));
			...
			*/
			pTransitionDuration = ((*(uint64_t*)((SadHook::GetImagebase() + 0x3B6FFD8)) + 0x280));
			...
			pMouseDisplacement = ((*(uint64_t*)((*(uint64_t*)((*(uint64_t*)(SadHook::GetImagebase() + 0x3BF6180)) + 0x30)) + 0x0)) + 0x0);
			...
			pRenderConfig = ((*(uint64_t*)((*(uint64_t*)((*(uint64_t*)((*(uint64_t*)(SadHook::GetImagebase() + 0x3627AE8)) + 0x0)) + 0x18)) + 0x8)) + 0x0);
			...
			if (GetAsyncKeyState(VK_F7)) {
				if (!bHQ) {
					...
					*/
					...
					Sleep(600);       
					SadMemEdit::WriteBytes({ 0xE8, 0x85, 0xC5, 0x02, 0x00 }, pCallRenderFrameSubProc); //Continue rendering.
					...
					*/
					...
					Sleep(500);
					SadMemEdit::WriteBytes({ 0xE8, 0x85, 0xC5, 0x02, 0x00 }, pCallRenderFrameSubProc); //...
