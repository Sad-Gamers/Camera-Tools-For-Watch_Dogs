...
void CameraSystem::EnableFreeCam(bool bFreeCamMode) {
	if (bFreeCamMode) {
		...
		*/
		...
		//Disrupt_b64.dll+1836B3A - E8 5162C2FE          
		SadMemEdit::WriteBytes({ 0x8B, 0x47, 0x30, 0xFF, 0xC8 }, DisruptHook::pReadActionMapEnum);        
		SadMemEdit::WriteBytes({ 0xE8, 0x51, 0x62, 0xC2, 0xFE }, DisruptHook::pCallMoveReticleSubProc);
	...
	while (true) {
		...
		*/
		if (DisruptHook::bCameraLoaded && !DisruptHook::bGameLoading) {
			//Update Pointers
			DisruptHook::pCameraStruct = ((*(uint64_t*)((*(uint64_t*)((*(uint64_t*)(SadHook::GetImagebase() + 0x3626F30)) + 0x0)) + 0x10)) + 0x0);
			...
			DisruptHook::pLensStruct = ((*(uint64_t*)((*(uint64_t*)((*(uint64_t*)(SadHook::GetImagebase() + 0x3628FE0)) + 0x0)) + 0x18)) + 0x0);
