...
void CameraSystem::EnableFreeCam(bool bFreeCamMode) {
	if (bFreeCamMode) {
		...
		*/
		...
		SadMemEdit::WriteBytes({ 0x8B, 0x02, 0x89, 0x41, 0x04 }, DisruptHook::pCameraPositionSubProc);
		SadMemEdit::WriteBytes({ 0x48,0x8B,0xC4, 0x48,0x89,0x70,0x10 }, DisruptHook::pViewMatrixSubProc);
		SadMemEdit::WriteBytes({ 0x48,0x89,0x5C,0x24,0x20 }, DisruptHook::pLensRenderpassSubProc1);
		SadMemEdit::WriteBytes({ 0x48,0x89,0x58,0x18 }, DisruptHook::pLensRenderpassSubProc2);
		SadMemEdit::WriteBytes({ 0x48,0x89,0x5C,0x24,0x18 }, DisruptHook::pLensRenderpassSubProc3);
